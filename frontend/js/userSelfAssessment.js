document.addEventListener("DOMContentLoaded", function () {
    ...
    if (startTestButton.textContent.trim() === "Continue") {
      ...
      try {
        ...
        const testResponse = await fetch(
          "http://18.143.151.82:5250/api/v1/selfAssessment/test",
          {
            method: "GET",
            headers: {
        ...
        if (testResponse.ok) {
          ...
          ws = new WebSocket(
            `ws://18.143.151.82:5250/api/v1/selfAssessment/ws?token=${token}`
          );
      ...
      try {
        ...
        const response = await fetch(
          `http://18.143.151.82:5250/api/v1/selfAssessment/startTest?userID=${userID}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  async function fetchTests() {
    try {
      ...
      const response = await fetch(
        "http://18.143.151.82:5250/api/v1/selfAssessment/getAllTests",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    if (currentTestIndex + 1 < allTests.length) {
      ...
      if (storedResults[testID]) {
        try {
          ...
          const response = await fetch(
            "http://18.143.151.82:5250/api/v1/selfAssessment/saveTestResult",
            {
              method: "POST",
              headers: {
...
async function fetchUserAssessmentResults() {
  try {
    ...
    // Fetch user results
    const response = await fetch(
      `http://18.143.151.82:5250/api/v1/selfAssessment/getUserResults?user_id=${userId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
